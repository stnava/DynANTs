...
mdl<-lm( (imging) ~ 1 + Education + age +
  quadmodel*ClinicalPhenotype1 , data=subdf )
mdlinterp<-bigLMStats( mdl , 1.e-4 )
for (  i in 1:nrow(mdlinterp$beta.pval) ) {
  print( rownames(mdlinterp$beta.pval)[i] )
  print( min(p.adjust(mdlinterp$beta.pval[i,],method='fdr'),na.rm=T) )
}
##### now do a lmer on the whole mat
mdl<-lmer( (imging[,999]) ~ 1 + AgeatMRI + Education +
 quadmodel*ClinicalPhenotype1+(1|fid), data=subdf )
...
# myform<-as.formula( paste("imging~1+(1|fid)" ) )
# rimging<-residuals( lmer(myform,data=demog),type = c("response" ))
# derka
