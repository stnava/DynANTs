...
#    TDP=as.factor(subdemog$LikelyTDP), 
  ...
  # selector<-malf == labnum
  # vizBV[ selector ]<-1.0-pvsBV[jj]
  # vizBVrate[ selector ]<-percentChange[2]
  # vizLV[ selector ]<-1.0-pvsLV[jj]
  # vizLVrate[ selector ]<-percentChange[3]
  # vizN[ selector ]<-1.0-pvsN[jj]
  # vizNrate[ selector ]<-percentChange[4]
  # vizSV[ selector ]<-1.0-pvsSV[jj]
  # vizSVrate[ selector ]<-percentChange[5]
  }
}
```
...
for ( k in 2:nrow(qv) ) {
  ...
  for ( j in 1:length(siginds) )
    ...
    vizimg[ malf == labnum ]<-qrates[k,siginds[j]]
    }
