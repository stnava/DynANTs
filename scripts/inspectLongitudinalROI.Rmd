...
# do this in order to make time more interpretable wrt quadratic
  ...
  myform<-as.formula( img ~ 1 + Age + Sex + ProfEdu +
    poi_dTime*poi_DX + dT2*poi_DX + (1+poi_dTime|sId) ) 
...
#    plot( effect("poi_dTime*poi_DX",mdl))
    locdf2=locdf
    locdf2$img<-residuals( lmer( img ~  (1+poi_dTime|sId), data=locdf ))
    myform2<-as.formula( img ~ 1 + Age + Sex + Edu + poi_dTime*poi_DX) 
    mdl2<-lm( myform2 , data=(locdf2))
    visreg(mdl2,"poi_dTime",by='poi_DX')
  }
